...
public enum ConfigPresets {
    ...
    RESET_MATERIALS("Reset Materials", (ResoundingConfig c) -> ConfigChanger.changeConfig(c, true,
            ...
            //<editor-fold desc="Map.ofEntries(),">
            Map.<String, MaterialData>ofEntries(
                    entry("Coral", new MaterialData(0.2,                  0.45 )),    // Coral              (coral_block)
                    entry("Gravel, Dirt", new MaterialData(0.5,                  0.4  )),    // Gravel, Dirt       (gravel, rooted_dirt)
                    entry("Amethyst", new MaterialData(0.75,                 0.45 )),    // Amethyst           (amethyst_block, small_amethyst_bud, medium_amethyst_bud, large_amethyst_bud, amethyst_cluster)
                    entry("Sand", new MaterialData(0.1,                  0.45 )),    // Sand               (sand)
                    entry("Candle Wax", new MaterialData(0.4,                  0.3  )),    // Candle Wax         (candle)
                    entry("Weeping Vines", new MaterialData(0.2,                  0.2  )),    // Weeping Vines      (weeping_vines, weeping_vines_low_pitch)
                    entry("Soul Sand", new MaterialData(0.05,                 0.65 )),    // Soul Sand          (soul_sand)
                    entry("Soul Soil", new MaterialData(0.1,                  0.7  )),    // Soul Soil          (soul_soil)
                    entry("Basalt", new MaterialData(0.8,                  0.3  )),    // Basalt             (basalt)
                    entry("Netherrack", new MaterialData(0.75,                 0.45 )),    // Netherrack         (netherrack, nether_ore, nether_gold_ore)
                    entry("Nether Brick", new MaterialData(0.85,                 0.55 )),    // Nether Brick       (nether_bricks)
                    entry("Honey", new MaterialData(0.08,                 0.85 )),    // Honey              (honey_block)
                    entry("Bone", new MaterialData(0.7,                  0.55 )),    // Bone               (bone_block)
                    entry("Nether Wart", new MaterialData(0.2,                  0.8  )),    // Nether Wart        (nether_wart, wart_block)
                    entry("Grass, Foliage", new MaterialData(0.2,                  0.6  )),    // Grass, Foliage  (grass, crop, bamboo_sapling, sweet_berry_bush)
                    entry("Metal", new MaterialData(0.85,                 0.5  )),    // Metal              (metal, copper, anvil)
                    entry("Aquatic Foliage", new MaterialData(0.15,                 0.8  )),    // Aquatic Foliage    (wet_grass, lily_pad)
                    entry("Glass, Ice", new MaterialData(0.5,                  0.45 )),    // Glass, Ice         (glass)
                    entry("Sculk Sensor", new MaterialData(0.4,                  0.6  )),    // Sculk Sensor       (sculk_sensor)
                    entry("Nether Foliage", new MaterialData(0.15,                 0.55 )),    // Nether Foliage     (roots, nether_sprouts)
                    entry("Shroomlight", new MaterialData(0.85,                 0.75 )),    // Shroomlight        (shroomlight)
                    entry("Chain", new MaterialData(0.4,                  0.4  )),    // Chain              (chain)
                    entry("Deepslate", new MaterialData(0.88,                 0.55 )),    // Deepslate          (deepslate)
                    entry("Wood", new MaterialData(0.65,                 0.45 )),    // Wood               (wood, ladder)
                    entry("Deepslate Tiles", new MaterialData(0.95,                 0.55 )),    // Deepslate Tiles    (deepslate_tiles)
                    entry("Stone, Blackstone", new MaterialData(0.83,                 0.5  )),    // Stone, Blackstone  (stone, calcite, gilded_blackstone)
                    entry("Slime", new MaterialData(1.0,                  0.25 )),    // Slime              (slime_block)
                    entry("Polished Deepslate", new MaterialData(0.99,                 0.55 )),    // Polished Deepslate (polished_deepslate, deepslate_bricks)
                    entry("Snow", new MaterialData(0.1,                  0.5  )),    // Snow               (snow)
                    entry("Azalea Leaves", new MaterialData(0.3,                  0.35 )),    // Azalea Leaves      (azalea_leaves)
                    entry("Bamboo", new MaterialData(0.5,                  0.4  )),    // Bamboo             (bamboo, scaffolding)
                    entry("Mushroom Stems", new MaterialData(0.6,                  0.65 )),    // Mushroom Stems     (stem)
                    entry("Wool", new MaterialData(0.02,                 1.0  )),    // Wool               (wool)
                    entry("Dry Foliage", new MaterialData(0.1,                  0.15 )),    // Dry Foliage        (vine, hanging_roots, glow_lichen)
                    entry("Azalea Bush", new MaterialData(0.15,                 0.5  )),    // Azalea Bush        (azalea)
                    entry("Lush Foliage", new MaterialData(0.2,                  0.2  )),    // Lush Foliage       (cave_vines, spore_blossom, small_dripleaf, big_dripleaf)
                    entry("Netherite", new MaterialData(1.0,                  0.6  )),    // Netherite          (netherite_block, lodestone)
                    entry("Ancient Debris", new MaterialData(0.45,                 0.8  )),    // Ancient Debris     (ancient_debris)
                    entry("Nether Fungus Stem", new MaterialData(0.3,                  0.55 )),    // Nether Fungus Stem (nether_stem)
                    entry("Powder Snow", new MaterialData(0.01,                 0.1  )),    // Powder Snow        (powder_snow)
                    entry("Tuff", new MaterialData(0.35,                 0.4  )),    // Tuff               (tuff)
                    entry("Moss", new MaterialData(0.1,                  0.85 )),    // Moss               (moss, moss_carpet)
                    entry("Nylium", new MaterialData(0.4,                  0.55 )),    // Nylium             (nylium)
                    entry("Nether Fungus", new MaterialData(0.4,                  0.6  )),    // Nether Fungus      (fungus)
                    entry("Lanterns", new MaterialData(0.75,                 0.4  )),    // Lanterns           (lantern)
                    entry("Dripstone", new MaterialData(0.9,                  0.6  )),    // Dripstone          (dripstone_block, pointed_dripstone)
                    entry("Default Material"    , new MaterialData(0.5,                  0.5  )),    // Default Material   ()
                    entry("block.minecraft.water"    , new MaterialData(0.5,    0.03 ))     // Water              (block.minecraft.water)
            ),//</editor-fold>
