...
public class LiquidStorage {
    ...
    public LiquidStorage(boolean @NotNull [][] s, int t, int b, boolean[] sf, WorldChunk ch ){
        int n = t-b+1; if (s.length != n || sf.length != n) ResoundingEngine.LOGGER.error("Top("+t+") to Bottom("+b+") != "+s.length+" or "+sf.length);
    ...
    public void setBlock(int x, int y, int z, boolean block) { // rare ⇒ can be expensive
        if (x >= 16 || x < 0 || z >= 16 || z < 0 || y >= 320 || y < -64) ResoundingEngine.LOGGER.error("Block coords ["+x+", "+y+", "+z+"] are out of bounds!");
