...
public class Ray {
    ...
    public @Nullable Segment reflected = null;
    public @Nullable Segment transmitted = null;
...
//    public static Material air = Cache.material(Blocks.AIR.getDefaultState());
    ...
    public Ray(@NotNull World world, @Nullable Branch tree, @Nullable ChunkChain chunk, @Nullable Vec3d targetPos) {
        this.world = world;
        this.tree = tree;
        this.chunk = chunk;
    ...
    public Ray(@NotNull World world, @Nullable Branch tree, @Nullable ChunkChain chunk) {
        this(world, tree, chunk, null);
    }
    //* raycast {
    public void cast(@NotNull Vec3d position, @NotNull Vec3d angle) {
        // TODO: settings.rayStrength & volume -> amplitude
        cast(position, angle, 128);
    }
    public void cast(@NotNull Vec3d position, @NotNull Vec3d vector, double power) {
        //* access branch {
    ...
    public void blank(Vec3d position) {
        this.reflected = new Segment(0, position, null, 0);
        this.transmitted = new Segment(0, position, null, 0);
    }
    private void reflect(/*MaterialData material,*/ double power, Vec3d position, Vec3d angle, double distance) {
        this.reflected = new Segment(power, position, angle, distance);
    }
    private void transmit(/*MaterialData material,*/ double power, Vec3d position, Vec3d angle, double distance) {
        this.transmitted = new Segment(power, position, angle, distance);
    }
    // } */
}
